...
class BossSpider(BaseSpider):
    ...
    def search(self, keyword: str, city: str) -> bool:
        ...
        # 构建搜索URL
        search_url = f"{self.base_url}/web/geek/job?query={quote(keyword)}&city={city_code}"
        
        # 导航到搜索页面
        self.engine.navigate(search_url)
        
        # 等待职位列表加载
        return self.engine.wait_for_element('.job-list-box', timeout=10)
    ...
    def parse_job_list(self) -> List[Job]:
        ...
        # 获取职位卡片列表
        job_cards = self.engine.get_elements('.job-list-box .job-card-wrapper')
    ...
    def _parse_job_card(self, card) -> Job:
        ...
        """
        # 获取职位ID
        job_link = card.ele('css:.job-title')
        if not job_link:
            return None
        
        href = job_link.attr('href') or ''
        job_id_match = re.search(r'/job_detail/([a-zA-Z0-9]+)\.html', href)
        if not job_id_match:
            return None
        ...
        # 获取薪资
        salary_elem = card.ele('css:.salary')
        salary = salary_elem.text.strip() if salary_elem else None
        
        # 获取公司名称
        company_elem = card.ele('css:.company-name a')
        company = company_elem.text.strip() if company_elem else "未知公司"
        
        # 获取工作地点
        location_elem = card.ele('css:.job-area')
