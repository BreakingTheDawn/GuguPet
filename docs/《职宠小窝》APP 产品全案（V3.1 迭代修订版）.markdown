# 《职宠小窝》APP 产品全案（V3.2 迭代修订版）

**文档说明**：本版本（V3.2）在 V3.1 基础上，新增"宠物私人空间"功能模块（小程序优先），包含所有已确认的产品定位、功能设计、技术架构、商业模式及美术规范，无任何待定项，可直接用于开发、美术对接、项目落地及团队同步，兼容Word、WPS导出，支持编辑修改。
...
宠物在用户未进入时处于随机待机状态，模拟宠物自己的"生活"：

| 状态名称 | 触发权重 | 持续时间 | 位置描述 | 动画效果 |
|----------|----------|----------|----------|----------|
| 睡觉 | 20% | 10-30秒 | 左下角床铺 | 呼吸起伏+Z字飘出 |
| 看书 | 18% | 8-20秒 | 书桌位置 | 翻页+点头 |
| 吃东西 | 15% | 5-15秒 | 餐桌位置 | 咀嚼+满足表情 |
| 玩耍 | 15% | 6-18秒 | 中央区域 | 跳跃+追逐玩具 |
| 散步 | 12% | 8-25秒 | 移动路径 | 行走+东张西望 |
| 发呆 | 20% | 5-15秒 | 随机位置 | 发呆+气泡幻想 |

#### 3.2.7.4 进入动画序列

用户进入宠物栏时，宠物执行"发现用户→走向屏幕前"的动画序列：
...
| 阶段 | 动画名称 | 持续时间 | 视觉效果 |
|------|----------|----------|----------|
| 1 | 发现用户 | 300-500ms | 轻微放大、抬头、睁眼 |
| 2 | 转头 | 400-600ms | 转向屏幕中心 |
| 3 | 走向用户 | 600-1000ms | 移动到屏幕中央偏上 |
| 4 | 准备倾听 | 200-400ms | 恢复正常姿态，等待输入 |
...
| 层级 | z-index | 内容 | 说明 |
|------|---------|------|------|
| 背景层 | 1 | 天空渐变、地板 | 场景氛围基础 |
| 装饰层 | 2 | 家具、植物、玩具 | 增加生活气息 |
| 宠物层 | 3 | 宠物主体 | 核心展示层 |
| 前景层 | 4 | 粒子特效、气泡 | 动态效果 |
| UI层 | 5 | 交互按钮 | 用户操作 |

**默认场景**：温馨小窝（暖色调渐变背景，包含床铺、书架、食物碗、植物等装饰）

#### 3.2.7.6 技术实现要点

- **状态机管理**：使用状态机管理宠物的状态流转，确保状态转换合法
- **CSS 3D动画**：通过CSS 3D变换实现转头、移动等动画效果
- **性能优化**：根据设备性能动态调整动画复杂度，保证流畅度
- **详细设计文档**：参见 `docs/miniprogram/PET_SPACE_宠物私人空间设计_v1.0.md`
